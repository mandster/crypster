...
interface TradeEntry {
  ...
  positionSize: string;
  symbol: string; // Ensure symbol is part of TradeEntry
}
...
const TradingJournal: React.FC<TradingJournalProps> = ({
  ...
  // Apply panel background and border colors using dark: variants
    ...
    <div className={panelClasses}>
      ...
      <div className="overflow-auto max-h-[400px] flex-grow">
        {tradeJournal.length === 0 ? (
          ...
          <table className="min-w-full text-sm text-gray-900 dark:text-gray-100"> {/* Default text color for table */}
            <thead>
            ...
            <tbody>
              {tradeJournal.map((trade) => (
                <tr key={trade.id} className="border-t border-gray-600 dark:border-gray-200">
                  ...
                  <td className="px-2 py-2 whitespace-nowrap">{trade.symbol}</td> {/* Display Symbol */}
